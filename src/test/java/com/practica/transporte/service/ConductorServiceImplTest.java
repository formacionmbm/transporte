...
import com.practica.transporte.entities.LicenciaConductor;
...
class ConductorServiceImplTest {
    ...
    void setUp() {
        ...
        LicenciaConductor licencia = new LicenciaConductor();
        licencia.setId(100L);
        licencia.setTipoLicencia("D");
        licencia.setCaducidadLicencia(LocalDate.now().plusYears(5));
        ...
        conductor.setDisponible(1);
        conductor.setEstadoConductor(EstadoConductor.ACTIVO);
    ...
    void testFindByNumeroEmpleado_Found() {
        when(conductorRepository.findByNumEmpleado("TEST001"))
                .thenReturn(Optional.of(conductor));
        when(conductorMapper.toDTO(conductor))
        ...
        verify(conductorRepository, times(1))
                .findByNumEmpleado("TEST001");
        verify(conductorMapper, times(1))
                .toDTO(conductor);
    }

    @Test
    void testFindByNumeroEmpleado_NotFound() {
        when(conductorRepository.findByNumEmpleado("NOEXISTE"))
        ...
        assertTrue(exception.getMessage()
                .contains("Conductor no encontrado"));

        verify(conductorRepository, times(1))
                .findByNumEmpleado("NOEXISTE");
        verify(conductorMapper, never())
                .toDTO(any());
    }
